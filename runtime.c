...
/*
  Allocates a number of zeroed bytes.
 */
void *_minijavalib_allocbytes (size_t n) {
  char *o = malloc (n);
  int i;
...
/*
   Prints a boolean to the standard output stream.

   Param i: The `boolean` to be printed.
*/
void _minijavalib_println_bool (int i)
{
  if (i) {
    printf ("true\n");
  } else {
